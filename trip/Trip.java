...
class Trip {
        ...
        try {
            ...
            while (inp.hasNext()) {
                ...
                case "R":
                    ...
                    break;
                default:
                    System.err.printf("map entry #%d: unknown type", n);
                    break;
                }
            }
            ...
            System.err.printf("bad entry #%d", n);
        } catch (NoSuchElementException excp) {
            System.err.printf("entry incomplete at end of file");
        }
    }
    ...
    void makeTrip(List<String> dests) {
        if (dests.size() < 2) {
            System.err.printf("must have at least two locations for a trip");
        }
        ...
        for (int i = 1; i < dests.size(); i += 1) {
            ...
            if (from == null) {
                System.err.printf("No location named %s", dests.get(i - 1));
            } else if (to == null) {
                System.err.printf("No location named %s", dests.get(i));
            }
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        String output1 = "%d" + ". Take %s %s for %.1f miles.";
        String output2 = "%d" + ". Take %s %s for %.1f miles to %s.";
        Iterator<Integer> segIter = segment.iterator();
        int a = segIter.next();
        int b = segIter.next();
        int c = 0;
        String currDir = "";
        Road nextRoad = _map.getLabel(a, b);
        while (segIter.hasNext()) {

            Road firstRoad =  nextRoad;
            currDir = firstRoad.direction().fullName();
            c = segIter.next();
            nextRoad = _map.getLabel(b, c);
            String name1 = firstRoad.direction().fullName();
            String name2 = nextRoad.direction().fullName();
            if (firstRoad.toString().equals(nextRoad.toString())
                    && name1.equals(name2)) {
                System.out.println(String.format(output1, seq,
                        firstRoad.toString(), firstRoad.direction().fullName(),
                        firstRoad.length() + nextRoad.length()));
                a = b;
                b = c;
                c = segIter.next();
                nextRoad = _map.getLabel(b, c);
            } else {
                System.out.println(String.format(output1,
                        seq, firstRoad.toString(),
                        firstRoad.direction().fullName(), firstRoad.length()));
            }
        ...
        System.out.println(String.format(output2, seq, nextRoad.toString(),
                currDir, nextRoad.length(), _map.getLabel(c).toString()));
        seq += 1;
    ...
    private void addLocation(String name, double x, double y) {
        if (_sites.containsKey(name)) {
            System.err.printf("multiple entries for %s", name);
        }
    ...
    private void addRoad(String from, String name, double length,
        ...
        if (v0 == null) {
            System.err.printf("location %s not defined", from);
        } else if (v1 == null) {
            System.err.printf("location %s not defined", to);
        }

        Road myRoad1 = new Road(name, dir, length);
        Road myRoad2 = new Road(name, dir.reverse(), length);
        _map.add(v0, v1, myRoad1);
        _map.add(v1, v0, myRoad2);
    ...
    private class TripPlan extends SimpleShortestPaths {
        ...
        protected double getWeight(int u, int v) {
            ...
            Road myRoad = _map.getLabel(u, v);

            return myRoad.length();
        }
